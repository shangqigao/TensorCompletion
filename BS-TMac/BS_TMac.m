function [X,Y,Out] = BS_TMac(data,known,Nway,coreNway,alpha,t,opts)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Tensor completion by combining TMac with our balancing scheme
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Input:
...
N = length(Nway); coNway = zeros(1,N);N1=Nway;
for n = 1:N
    Nway(n)=prod(N1(t{n}));
    coNway(n) = prod(N1)/Nway(n);
end
nrmb = norm(data);
...
for k = 1:maxit
    ...
    errList(k) =  norm(M(known)-data) / norm(data);
