...
%% code 
function [Y, errList] = BS_FaLRTC(M, Omega, alpha, mu, L, C, maxIter,epsilon,data,known,t, X)
...
for i = 1:N
    ...
    tmp(i) = max(SingularValue(matricize(X,t{i}))) * alpha(i) * 0.3;
end
P = 1.15;
flatNum = 15;
...
for k = 1:maxIter
    ...
    while true
        ...
        fx = 0;
        for i = 1 : N
            [temp, sigma2] = Truncate(matricize(X,t{i}), ma(i));
        ...
        fy = 0;
        for i = 1 : N
            [sigma] = SingularValue(matricize(Y,t{i}));
...
%     end
    ...
    Z = Z - b*Gx;
    B = B+b;
    errList(k) =  norm(Ylast(known)-data) / norm(data);
...
%%%%%%%%
errList = errList(1:k);
fprintf('BS_FaLRTC ends: total iterations = %d   difference=%f\n\n', k, errList(k));
