%% Smooth low-rank tensor completion via combining nuclear norm with matrix factorization
function[Y]=S_LRTC(M, Omega,mu, L, C, maxIter, epsilon,coreNway,alpha,opts,X1)
% initialization
if nargin < 11
    X1= M;
...
for i = 2:2:N
    ...
    tmp(i) = max(SingularValue(Unfold(X1, dim, i))) * alpha(i) * 0.3;
end
P = 1.15;
flatNum = 15;
...
for k = 1:maxIter
    ...
    while true
        ...
        fx = 0;
        for i = 2 :2: N
            [temp, sigma2] = Truncate(Unfold(X1, dim, i), ma(i));
        ...
        fy = 0;
        for i = 2 :2: N
            [sigma] = SingularValue(Unfold(Y1, dim, i));
